...
class CadastroVagasDAO extends PDOConnectionFactory {
	...
	public function Insere($CadastroVagas) {
		try {
			...
			$stmt = $this->conex->prepare ( "INSERT INTO vaga (areaVaga,grauEstudo,requisitos, descricao, salario, beneficios) VALUES (?, ?, ?, ?,?,?)" );
			...
			$stmt->bindValue ( 6, $CadastroVagas->getBeneficios () );
	...
	public function Update($CadastroVagas, $condicao) {
		try {
			...
			$stmt = $this->conex->prepare ( "UPDATE cadastroVagas SET areaVaga=?, grauEstudo=?, requisitos=?, descricao=?, salario=?, beneficios=? WHERE id=?" );
			
			$this->conex->beginTransaction ();
			
			// valores encapsulados nas variáveis
